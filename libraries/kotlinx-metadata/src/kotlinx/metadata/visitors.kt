...
abstract class ClassVisitor(delegate: ClassVisitor? = null) : DeclarationContainerVisitor(delegate) {
    ...
    open fun visit(flags: Int, fqName: String) {
        delegate?.visit(flags, fqName)
    }
    ...
    open fun visitSealedSubclass(fqName: String) {
        delegate?.visitSealedSubclass(fqName)
    }
...
abstract class TypeVisitor(private val delegate: TypeVisitor? = null) {
    ...
    open fun visitClass(fqName: String) {
        delegate?.visitClass(fqName)
    }

    open fun visitTypeAlias(fqName: String) {
        delegate?.visitTypeAlias(fqName)
    }
...
class Annotation(val classFqName: String, val arguments: Map<String, AnnotationArgument<*>>)
...
sealed class AnnotationArgument<out T : Any> {
    ...
    data class KClassValue(override val value: String) : AnnotationArgument<String>()
    data class EnumValue(val enumClassName: String, val enumEntryName: String) : AnnotationArgument<String>() {
