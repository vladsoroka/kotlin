...
// TODO: validate as much as we can, e.g. JVM signatures should contain "("
...
class WriteContext {
    ...
    operator fun get(string: String): Int {
        // TODO (!): support local class names
        // TODO: use getQualifiedClassNameIndex for more optimized storage of class names
        return strings.getStringIndex(string)
    }
}
...
private fun writeType(c: WriteContext, flags: Int, ext: TypeVisitor.Extensions, output: (ProtoBuf.Type.Builder) -> Unit): TypeVisitor =
    object : TypeVisitor() {
        ...
        override fun visitClass(fqName: String) {
            t.className = c[fqName]
        }

        override fun visitTypeAlias(fqName: String) {
            t.typeAliasName = c[fqName]
        }
...
open class ClassWriter : ClassVisitor() {
    ...
    override fun visitSealedSubclass(fqName: String) {
        t.addSealedSubclassFqName(c[fqName])
    }
