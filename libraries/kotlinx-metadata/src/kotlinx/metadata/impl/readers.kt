...
// TODO: review all "optional" fields and prevent default messages from being used; do not call visit at all in this case
...
class ReadContext(
    ...
    operator fun get(index: Int): String {
        // On JVM, getString and getQualifiedClassName work exactly the same
...
fun ProtoBuf.Class.accept(v: ClassVisitor, outer: ReadContext) {
    val c = outer.withTypeParameters(typeParameterList)

    v.visit(flags, c[fqName])
    ...
    for (sealedSubclassFqName in sealedSubclassFqNameList) {
        v.visitSealedSubclass(c[sealedSubclassFqName])
    }

    if (hasVersionRequirement()) {
        v.visitVersionRequirement()?.let {
            val requirement = VersionRequirement.create(this, c.strings, c.versionRequirements)
                    ?: throw InconsistentKotlinMetadataException("No VersionRequirement with the given id in the table")
            requirement.accept(it)
        }
    }
...
fun ProtoBuf.Package.accept(v: PackageVisitor, c: ReadContext) {
    v.visitDeclarations(functionList, propertyList, typeAliasList, c)
...
fun ProtoBuf.Function.accept(v: LambdaVisitor, c: ReadContext) {
...
private fun ProtoBuf.Type.accept(v: TypeVisitor, c: ReadContext) {
    if (hasClassName()) {
        v.visitClass(c[className])
    }

    if (hasTypeAliasName()) {
        v.visitTypeAlias(c[typeAliasName])
    }

    if (hasTypeParameter()) {
        v.visitTypeParameter(typeParameter)
    } else if (hasTypeParameterName()) {
    ...
    for (argument in argumentList) {
        ...
        if (variance != null) {
            argument.type(c.types)?.let { argumentType ->
...
private fun VersionRequirement.accept(v: VersionRequirementVisitor) {
    val kind = when (kind) {
